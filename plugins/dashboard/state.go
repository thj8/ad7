...
// getCompetitionState 获取比赛的完整状态数据。
// 按顺序组装：比赛信息 → 题目状态（含解题数和一血）→ 排行榜 → 统计 → 最近事件。
func (p *Plugin) getCompetitionState(ctx context.Context, compID string) (*stateResponse, error) {
	...
	// 5. 获取最近事件（从内存缓存中过滤当前比赛）
	resp.RecentEvents = p.getRecentEventsForComp(compID)

	return resp, nil
}

// getCompetitionInfo 从数据库查询比赛基本信息。
...
// getChallengeStates 获取比赛中所有题目的状态（含解题数和一血信息）。
// 分三步查询：题目列表 → 解题数统计 → 一血信息，然后合并到结果中。
...
// getLeaderboard 获取比赛的排行榜数据。
// 按用户分组，计算总分和最后解题时间。总分降序，同分按最后解题时间升序。
...
// getRecentEventsForComp 从内存中获取指定比赛的最近事件。
// 当前实现返回所有事件（简化处理），后续可按比赛 ID 精确过滤。
func (p *Plugin) getRecentEventsForComp(compID string) []recentEvent {
	allEvents := p.getRecentEvents()
	// 当前简化处理，返回全部事件；实际可按 compID 精确过滤
	return allEvents
}

// getFirstBloodList 查询指定比赛的所有一血记录。
// 关联题目表获取标题、分类和分值，按一血获得时间升序排列。
