...
- **全局和模块级开关**: 缓存系统必须支持全局启用/禁用，以及按模块级别的启用/禁用
- **NoOpProvider**: 提供无操作的缓存提供器实现，使得在禁用缓存时不需要修改业务代码
- **前缀批量失效**: 优先使用前缀批量清除缓存，而不是单个 key 清除，这样更高效且不容易遗漏
- **依赖方向**: 基础插件（如 cache）不应该依赖业务插件（如 topthree），依赖关系应该是单向的
- **缓存失效职责**: 每个插件负责管理自己的缓存失效，不要跨插件管理缓存
- **快速路径优化**: 对于频繁访问的数据，先查缓存，缓存命中时直接返回，避免不必要的数据库查询
- **缓存键设计**: 单条数据和集合数据使用不同的缓存键模式，按需刷新
- **配置优先**: 缓存参数（TTL、清理间隔等）应该从配置文件读取，不要硬编码

## 代码质量规则（本次会话新增）

- **DRY 原则**: 提取公共函数减少重复代码，相同逻辑不要写两遍
- **本地接口**: 在使用其他插件的功能时，定义本地接口只包含需要的方法，减少依赖
- **向后兼容**: 重构时保持向后兼容，不要破坏现有功能
- **先审查再修改**: 在修改代码前先理解现有设计，不要盲目动手
